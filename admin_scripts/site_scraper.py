...
class PageLevelData:
    def __init__(self, level_num: int, previous_level_name: str,
                 previous_level_coords: tuple[str, str], image_urls: list):
...
def main(save_off_directory="."):
    save_off_directory = Path(save_off_directory)
    levels = []
...
if __name__ == '__main__':
    ...
    argparser.add_argument("cookie", help="Site cookie, in format "
                                          "'csrftoken=<foo>; sessionid=<bar>'")
    argparser.add_argument("save_dir", help="Path into a directory into which to save"
                                            " the levels. Script will create it "
                                            "if it doesn't exist.")
    argparser.add_argument("minlevel", help="Minimum level number, usually 1")
    argparser.add_argument("maxlevel", help="Maximum level number. Script will actually"
